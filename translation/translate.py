    ...
    with tf.gfile.GFile(source_path, mode="r") as source_file:
        with tf.gfile.GFile(target_path, mode="r") as target_file:
            ...
            while source and target and (not max_size or counter < max_size):
                counter += 1
                if counter % 10000 == 0:
                    handleInfo(str("Reading data line " + str(counter)))
                    sys.stdout.flush()
...
def train():
    ...
    with tf.Session() as sess:
        ...
        handleInfo(str("Reading development and training data (limit: "+str(FLAGS.max_train_data_size)+")."))
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                if perplexity < 9.99:
                    if not _lowestPerplexity:
                        _lowestPerplexity.append(perplexity)
                        _lowestPerplexity.append(0)

                    if perplexity == _lowestPerplexity[0]:
                        if _lowestPerplexity[1] > 10:
                            break
                        else:
                        ...
                        if perplexity < _lowestPerplexity[0]:
                            _lowestPerplexity[0] = perplexity
                            _lowestPerplexity[1] = 1

                    #c = c + 1
                                        #c = c + 1
                    handleInfo(str("Lowest Perplexity List--" + str(_lowestPerplexity)))
                message = "global step "+str(model.global_step.eval())+" learning rate "+str(model.learning_rate.eval())+" step-time "+str(step_time)+" perplexity " + str(perplexity)
                handleInfo(message)
...
def decode():
    with tf.Session() as sess:
        ...
        es_vocab_path = os.path.join(FLAGS.data_dir,
                                     "vocab%d.es" % FLAGS.from_vocab_size)
        en_vocab_path = os.path.join(FLAGS.data_dir,
                                     "vocab%d.en" % FLAGS.to_vocab_size)
...
def self_test():
    ...
    with tf.Session() as sess:
        handleInfo("Self-test for neural translation model.")
